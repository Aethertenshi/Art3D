...
static const std::vector<std::string> wrenKeywordsList = {
...
void Render3DObject(GameObject* obj, SDL_GPURenderPass* renderPass, SDL_GPUCommandBuffer* cmd, const Matrix4x4& view, const Matrix4x4& proj, GpuPipeline& pipeline, const LightUBO& lightData, const std::vector<std::shared_ptr<GameObject>>& sceneObjects) {
    if (!obj) return;
    if (obj->Is3D) {
        Vector3D worldPos = obj->GetWorldPosition();
        Vector3D worldRot = obj->GetWorldRotation();
        ...
        LightUBO customLightData = lightData;
        if (obj->ClassName != "LightSource" && IsObjectShadowed(obj, sceneObjects, Vector3D(lightData.pos[0], lightData.pos[1], lightData.pos[2]))) {
            customLightData.intensity = 0.0f;
        }
        ...
        SDL_PushGPUFragmentUniformData(cmd, 0, &customLightData, sizeof(customLightData));
    ...
    // Traverse children
    for (auto& child : obj->Children) {
        Render3DObject(child.get(), renderPass, cmd, view, proj, pipeline, lightData, sceneObjects);
    }
}
...
int main() {
    ...
    // Load JetBrains Mono fonts from the ttf directory using robust path resolution
    std::string fontPath = ResolveAssetPath("ttf/JetBrainsMono-Regular.ttf");
    ImFont* defaultFont = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 15.0f);
    if (!defaultFont) {
        io.Fonts->AddFontDefault();
    }
    ImFont* scriptFont = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 17.0f);
    ...
    while (running) {
        ...
        // Handle shortcut keys (Ctrl+I to open Import Popup)
        ...
        if (ImGui::BeginPopupModal("Import Object", &showImportModal, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            ImGui::TextDisabled("2D UI Elements");
            if (ImGui::Button("2D Frame (User Interface)", ImVec2(250, 0))) {
                auto newObj = std::make_shared<Frame>("Frame", Position2D(0, 0, 100, 100), Size2D(0, 0, 200, 150));
                addObject(newObj);
                showImportModal = false;
                ImGui::CloseCurrentPopup();
            }
            if (ImGui::Button("2D GameObject (UI Panel)", ImVec2(250, 0))) {
            ...
            ImGui::TextDisabled("Scripts & Logic");
            if (ImGui::Button("Logic Script", ImVec2(250, 0))) {
            ...
            ImGui::TextDisabled("Import External Models");
            static char objPathBuf[256] = "suzanne.obj";
            ImGui::InputText("OBJ File Path", objPathBuf, sizeof(objPathBuf));
            if (ImGui::Button("Import wavefront .obj", ImVec2(250, 0))) {
        ...
        if (g_showScriptEditor && !g_openScripts.empty()) {
            ...
            if (ImGui::Begin("Script Editor", &g_showScriptEditor, ImGuiWindowFlags_MenuBar)) {
                ...
                if (ImGui::BeginMenuBar()) {
                    ImGui::Text("Script Editor (Font Size: %.0fpx)", scriptFontSize);
                    ImGui::EndMenuBar();
                }
                ...
                if (ImGui::BeginTabBar("ScriptTabs", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_AutoSelectNewTabs)) {
                    ...
                    for (size_t i = 0; i < g_openScripts.size(); i++) {
                        ...
                        if (ImGui::BeginTabItem(tabLabel.c_str(), &open, tabFlags)) {
                            ...
                            if (scriptFont) ImGui::PushFont(scriptFont, scriptFontSize);
                            editor.Render("##codeEditor");
                            if (scriptFont) ImGui::PopFont();
                            ...
                            if (!currentWord.empty() && std::isalpha((unsigned char)currentWord[0])) {
                                g_autocompleteMatches.clear();
                                for (const auto& keyword : wrenKeywordsList) {
        ...
        if (cmd) {
            ...
            if (SDL_WaitAndAcquireGPUSwapchainTexture(cmd, window, &swapchainTexture, &w, &h) && swapchainTexture != nullptr) {
                ...
                if (renderPass) {
                    ...
                    LightUBO lightData = {};
                    lightData.pos[0] = 0.0f; lightData.pos[1] = 3.0f; lightData.pos[2] = 0.0f;
                    lightData.intensity = 1.0f;
                    lightData.color[0] = 1.0f; lightData.color[1] = 1.0f; lightData.color[2] = 1.0f;
                    for (auto& obj : sceneObjects) {
                        if (obj->ClassName == "LightSource") {
                            LightSource* light = static_cast<LightSource*>(obj.get());
                            Vector3D worldPos = light->GetWorldPosition();
                            lightData.pos[0] = worldPos.X;
                            lightData.pos[1] = worldPos.Y;
                            lightData.pos[2] = worldPos.Z;
                    ...
                    // Render 3D Handles / Gizmo for selected object (if any)
                    if (selectedObject) {
                        ...
                        Handle handles[] = {
                            // X-axis (Red)
                            { Vector3D(0.5f, 0.02f, 0.02f), Vector3D(0.35f, 0.0f, 0.0f), { 1.0f, 0.1f, 0.1f, 1.0f } }, // Shaft
                            { Vector3D(0.06f, 0.06f, 0.06f), Vector3D(0.62f, 0.0f, 0.0f), { 1.0f, 0.1f, 0.1f, 1.0f } }, // Tip
                            // Y-axis (Green)
                            { Vector3D(0.02f, 0.5f, 0.02f), Vector3D(0.0f, 0.35f, 0.0f), { 0.1f, 0.9f, 0.1f, 1.0f } }, // Shaft
                            { Vector3D(0.06f, 0.06f, 0.06f), Vector3D(0.0f, 0.62f, 0.0f), { 0.1f, 0.9f, 0.1f, 1.0f } }, // Tip
                            // Z-axis (Blue)
                            { Vector3D(0.02f, 0.02f, 0.5f), Vector3D(0.0f, 0.0f, 0.35f), { 0.1f, 0.1f, 0.9f, 1.0f } }, // Shaft
                            { Vector3D(0.06f, 0.06f, 0.06f), Vector3D(0.0f, 0.0f, 0.62f), { 0.1f, 0.1f, 0.9f, 1.0f } }  // Tip
                        };
                        ...
                        for (const auto& handle : handles) {
                            ...
                            // Offset handles based on their axis directions
                            float ox = 0.0f;
                            float oy = 0.0f;
                            float oz = 0.0f;
                            if (handle.offset.X > 0.0f) ox = worldScale.X * 0.5f;
                            if (handle.offset.Y > 0.0f) oy = worldScale.Y * 0.5f;
                            if (handle.offset.Z > 0.0f) oz = worldScale.Z * 0.5f;

                            Matrix4x4 model = Matrix4x4::Scale(handle.scale.X, handle.scale.Y, handle.scale.Z)
                                .Multiply(Matrix4x4::Translation(
                                    worldPos.X + ox + handle.offset.X,
                                    worldPos.Y + oy + handle.offset.Y,
                                    worldPos.Z + oz + handle.offset.Z
                                ));
                            Matrix4x4 mvp = model.Multiply(view).Multiply(proj);
                            ...
                            vsUniform.MVP   = mvp;
                            vsUniform.Model = model;
                            vsUniform.ColorOverride[0] = handle.color[0];
                            vsUniform.ColorOverride[1] = handle.color[1];
                            vsUniform.ColorOverride[2] = handle.color[2];
                            vsUniform.ColorOverride[3] = handle.color[3];
